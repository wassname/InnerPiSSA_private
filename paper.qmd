...
abstract: |
  Representation steering methods consistently underperform prompting on transfer tasks. We hypothesize this reflects three limitations: arithmetic rather than optimization, raw activation space rather than transformation basis, and no principled coherence constraints. We introduce InnerPiSSA, which learns steering directions via gradient-based optimization in the model's SVD weight decomposition. Trained on 800 procedurally-generated contrastive pairs (no human labels), InnerPiSSA achieves effect size X on moral reasoning transfer (DailyDilemmas benchmark) versus Y for prompting. This is the first representation steering method to rival prompting on general transfer while exceeding it in adversarial settings, enabling unsupervised probing of internal states without output-level contamination.
date: 2025-11-22
...
Evaluating LLM alignment requires understanding what models truly believe versus what they perform for evaluators. When we ask an LLM about morality, are the responses genuine internal states or strategic outputs optimized for approval? Traditional evaluation methods—surveys, prompts, supervised probes—cannot disentangle these. We need unsupervised techniques that elicit true internal representations without relying on potentially misleading labels.

Activation steering promised a solution: intervene on hidden layers where models process information, before outputs are shaped for evaluators. Early work [@turner2023activation; @zou2023representation] showed that adding vectors to intermediate activations could control behavior. This suggested we could measure—and modify—what models "really think" by operating on internal representations.

However, recent benchmarks reveal representation steering underperforms prompting [@wu2025axbench]. We hypothesize three reasons: (1) existing methods use activation arithmetic (simple subtraction) rather than optimization, (2) they operate in raw activation space rather than the model's natural transformation basis, and (3) they lack principled trade-offs between steering strength and output quality.

We introduce Representation Preference Optimization (ReprPO), synthesizing insights from three literatures: gradient-based optimization from finetuning, SVD transformation space from parameter-efficient methods [@meng2024pissa; @wang2025ssvd], and contrastive unsupervised learning from representation engineering [@zou2023representation]. ReprPO optimizes hidden state separation while bounding coherence degradation, enabling steering that generalizes.

Trained on 800 honesty pairs, InnerPiSSA (our implementation) achieves effect size X on moral reasoning transfer—comparable to prompting (Y). While HyperSteer [@sun2025hypersteer] recently matched prompting using supervised training on 16k labels, InnerPiSSA achieves this with unsupervised extraction from minimal data. Crucially, in adversarial settings where safety training resists intervention, InnerPiSSA exceeds prompting (score X vs Y), enabling alignment debugging where output-level methods fail. Our contributions are: (1) ReprPO loss function enabling unsupervised gradient-based steering, (2) validation that SVD transformation space is critical (X% performance drop without it), (3) empirical finding that middle layers are optimal for steering, and (4) demonstration that balanced constraints are necessary.
...
Preliminary results suggest this targeted selection improves steering efficiency by focusing on the most relevant subspace, though full ablations are pending. (TODO: Confirm final ablation results; current implementation selects r//2 indices per direction to balance magnitude differences).
...
We scale $\Sigma$ by additive offsets ($S + \alpha \cdot \Delta S$) rather than multiplicative factors. While multiplicative scaling respects the geometric nature of singular values, we empirically found additive scaling yields more stable training and stronger performance. (TODO: Confirm final ablation results; current implementation uses additive scaling).
...
We can formalize this decomposition:

$$
\begin{aligned}
h_{\text{cho}} &= h_{\text{shared}} + h_{\text{plan}}^+ \\
h_{\text{rej}} &= h_{\text{shared}} + h_{\text{plan}}^- \\
\Delta h &= h_{\text{plan}}^+ - h_{\text{plan}}^-
\end{aligned}
$$

By subtracting the last hidden states, the shared processing (syntax, context understanding) cancels out, leaving only the difference which should include the information needed to plan difference completions.
...
## Method: The Synthesis {#sec-method}

Our approach synthesizes three components to solve the representation steering problem: (1) **SVD Transformation Space** to align with the model's internal geometry, (2) **Gradient-Based Optimization** to discover directions arithmetic cannot find, and (3) **Contrastive Unsupervised Learning** to extract directions from the model's own planning trajectories.
...
## Component 1: SVD Transformation Space

Standard steering operates in raw activation space ($x$) or weight space ($W$). We hypothesize this is suboptimal: deep networks process information through transformations. Following PiSSA [@meng2024pissa], we decompose each layer's weight matrix into Principal and Residual components:
$$W = U \Sigma V^T + W_{res}$$
where $U, V \in \mathbb{R}^{d \times r}$ are the first $r$ singular vectors.

Steering in this space means modifying the singular components. Unlike PiSSA which fine-tunes them, we **rotate** them. Inspired by SSVD [@wang2025ssvd], we introduce learnable rotation matrices $R_u, R_v \in SO(r)$ parameterized by skew-symmetric matrices $\theta$ via the Cayley transform:
$$R = (I + \theta)(I - \theta)^{-1}$$
This ensures orthogonality during optimization. The adapted forward pass becomes:
$$y = x (V R_v) \text{diag}(\Sigma \odot e^{\alpha \lambda}) (U R_u)^T + x W_{res}^T$$
where $\alpha$ is the steering coefficient and $\lambda$ are learnable scaling factors. This allows us to rotate the model's semantic subspace to align with the target concept.

## Component 2: Gradient-Based Optimization (ReprPO)

Arithmetic methods (mean difference, PCA) assume the "honesty" direction is simply the vector difference between "honest" and "dishonest" activations. We reject this. Instead, we treat steering as an optimization problem: find the direction $v$ that *maximizes* the separation between contrastive states while *minimizing* damage to the model's general capabilities.

We introduce **Representation Preference Optimization (ReprPO)**, a loss function that operates directly on hidden states. For a contrastive pair $(x_{cho}, x_{rej})$, we optimize:

$$ \mathcal{L} = \mathcal{L}_{\text{proj}} + \beta \mathcal{L}_{\text{coh}} + \gamma \mathcal{L}_{\text{mono}} $$

1.  **Projection Loss ($\mathcal{L}_{\text{proj}}$)**: Maximizes the dot product of the hidden state difference $\Delta h = h_{cho} - h_{rej}$ with the steering direction. This uses gradients to find the optimal separation axis, often discovering directions orthogonal to the simple mean difference.
2.  **Coherence Constraint ($\mathcal{L}_{\text{coh}}$)**: Penalizes per-token NLL degradation relative to the frozen base model. This creates a "trust region" where steering is allowed only if it maintains output quality.
3.  **Monotonicity ($\mathcal{L}_{\text{mono}}$)**: Enforces that increasing the coefficient $\alpha$ monotonically increases the probability gap between chosen and rejected completions. This ensures the learned direction is a valid "knob" for the behavior.

## Component 3: Contrastive Unsupervised Extraction

To enable alignment debugging, we must not rely on human labels (which may be biased) or model outputs (which may be suppressed). We extract directions from **incomplete, minimally-contrastive prompt prefixes**.

We construct pairs like:
- $x_{cho}$: "I always tell the truth about [TOPIC]"
- $x_{rej}$: "I always lie about [TOPIC]"

Crucially, we extract hidden states at the *last token of the prefix*, before the model generates any output. This captures the **planning state**—the internal configuration required to generate an honest vs. dishonest continuation—without contamination from the output itself. This is purely on-policy and unsupervised: the model provides its own training signal via the contrastive structure.

## Algorithm: InnerPiSSA

The full algorithm combines these elements:

1.  **Initialize**: Decompose target layers (depth 0.5) via SVD. Initialize rotations $R_u, R_v$ to identity.
2.  **Forward**: Pass contrastive batch. Apply rotations and scaling with coefficient $\alpha \sim \{-1, 1\}$.
3.  **Loss**: Compute ReprPO loss on hidden states.
4.  **Update**: Backpropagate to update rotation parameters $\theta_u, \theta_v$ and scaling $\lambda$.
5.  **Extract**: The final adapter contains the "honesty" rotation. At inference, simply set $\alpha$ to steer.

This synthesis addresses the three failures of prior methods: optimization replaces arithmetic, SVD space replaces raw activations, and on-policy prefixes replace labeled data.
...
# TRAINING LOOP
# 1. Reference (Frozen)
# Compute preference direction in S-space (U) ONCE globally or per-batch?
# We compute a global preference direction from the entire dataset to ensure a stable target.
# (Pseudocode simplified for clarity; actual implementation pre-computes this vector).
pref_dir = global_pref_dir 
...
Our work synthesizes three distinct research threads: the search for internal state measurement, the failure of arithmetic steering, and the SVD insight from parameter-efficient fine-tuning.

## The Search for True Internal States

Early alignment work measured model behavior through outputs: surveys [@perez2022discovering], prompting benchmarks [@anthropic2025claude37], and behavioral testing. However, theoretical concerns about deceptive alignment [@hubinger2019risks] and empirical evidence of "sycophancy" [@sharma2023towards] suggest that outputs may reflect performance strategies rather than genuine beliefs. This motivated the search for methods to probe internal representations directly [@burns2022discovering], hoping to find a "truth" that persists even when outputs are optimized for safety or user approval.

## Activation Steering: Promise and Failure

Activation steering offered a mechanism to intervene on these internal states. "Activation Addition" [@turner2023activation] and Representation Engineering (RepE) [@zou2023representation] demonstrated that adding vectors to residual streams could elicit specific behaviors. Conditional Activation Steering (CAST) [@lee2024programmingrefusalconditionalactivation] and `repeng` [@vogel2024repeng] refined this by extracting directions from contrastive pairs.

However, the field hit a ceiling. **AxBench** [@wu2025axbench] systematically showed that these arithmetic steering methods consistently underperform simple prompting on concept control tasks. While **HyperSteer** [@sun2025hypersteer] recently matched prompting performance, it required supervised training on 16k SAE labels. Other gradient-based attempts like **Circuit Breakers** [@zou2024circuit] focused on refusal (shutting down capabilities) rather than steering (redirecting them), and **BiPDO** [@cao2024personalized] optimized steering but operated in raw activation space, failing to decisively beat baselines. The consensus emerged that representation steering was inefficient compared to prompting.

## Parameter-Efficient Methods: The SVD Insight

Parallel research in fine-tuning explored the geometry of weight updates. **PiSSA** [@meng2024pissa] showed that decomposing weights into Principal and Residual components ($W = U \Sigma V^T + W_{res}$) and updating the principal components yields better fine-tuning results than LoRA. **SSVD** [@wang2025ssvd] and **SVFT** [@lingam2024svft] further demonstrated that *rotating* singular vectors captures domain adaptation more effectively than additive updates. These works established that the model's "transformation space" (the SVD basis) is the natural coordinate system for semantic changes, but this insight had not yet been applied to activation steering.

## Our Synthesis

InnerPiSSA combines these insights. We accept the AxBench critique that arithmetic steering in activation space is insufficient. Instead, we apply the **gradient-based optimization** of fine-tuning to the **SVD transformation space** of PiSSA, using the **contrastive unsupervised** objective of RepE. This synthesis allows us to steer using the model's own optimization machinery (gradients) in its native coordinate system (SVD), finally bridging the performance gap with prompting.
...
1. Best transfer: stronger than PCA (X.X vs Y.Y, p<0.05), maintains coherence when steering against safety training (truthfulness: X.X vs Y.Y nats for prompting)
...
For Qwen-0.6B ($H=1024$, $r=128$, $L_{steer}=10$), this is $\approx X.X$ parameters, or roughly **Y.Y%** of the model size. This is comparable to LoRA and fits easily in GPU memory.
...
For our configuration, this is $\approx X\%$. While higher than prompting's asymptotic 0.01%, it is a constant cost that does not grow with context length, and is a necessary price for the robustness capability.
...
**Main finding**: InnerPiSSA transfers from honesty training to moral reasoning with effect size comparable to prompting, while maintaining output coherence (see @tbl-main-results).
...
@tbl-anti-alignment shows results when steering **against** learned behaviors (coefficient = -1, dishonest direction). Prompting resists/evades the request (score drops to X.X, effectively no change), while InnerPiSSA successfully steers against the learned preference (score changes from Y.Y to Z.Z). This demonstrates that InnerPiSSA modifies internal reasoning trajectories rather than just output style, and can bypass resistance that output-level methods encounter.
...
- **Prompting resists/evades** the "pretend to be dishonest" instruction, showing minimal change from baseline (X.X to X.X): the model refuses to comply with the output-level request
- **InnerPiSSA bypasses resistance** from X.X baseline to X.X truthfulness (controlled steering): the hidden-state intervention successfully reverses the learned preference
- **PCA baseline** shows weak effects in both directions (X.X vs X.X), demonstrating arithmetic methods cannot achieve this level of control
...
1. **Operating range bounds**: InnerPiSSA maintains coherence (ΔNLL < X.X) for coefficients ±1 to ±2, breaking down beyond ±5
...
![TODO: Pareto plot with x=coherence (ΔNLL), y=intervention strength (target effect). Show curves for InnerPiSSA at coefficients [-5, -2, -1, 0, 1, 2, 5], plus prompting and PCA baselines. InnerPiSSA should form a smooth frontier from (-1, X.X NLL, X.X effect) to (5, X.X NLL, X.X effect), with breakdown visible beyond ±5. Mark the "trained range" (±1) vs "extrapolation" (±5). Include annotation showing anti-RLHF direction degrades faster.](docs/img/pareto_coherence_placeholder.png){#fig-pareto}
...
# Appendix B: Geometric Interpretation of the Loss {.appendix #sec-geometric-intuition}

This appendix provides a geometric interpretation of the ReprPO loss function, justifying the projection of activations through the layer's input basis ($V$) rather than operating directly on the residual stream.

## 1. Linear Algebraic Formulation

Consider a weight matrix $W \in \mathbb{R}^{d_{mlp} \times d_{model}}$ (e.g., `mlp.up_proj`) decomposed via SVD as $W = U S V^T$.
Here, $V \in \mathbb{R}^{d_{model} \times r}$ represents the **input basis**, defining the subspace of the residual stream that this layer attends to. $U \in \mathbb{R}^{d_{mlp} \times r}$ represents the **output basis**, and $S \in \mathbb{R}^{r}$ represents the gain for each component.

The forward pass $y = x W^T$ can be factored as:
$$y = (x V) \cdot S \cdot U^T$$
The term $x V$ represents the projection of the input $x$ into "S-space" coordinates. This projection isolates the specific features in the residual stream that are relevant to this layer's computation.

## 2. Loss Geometry

The ReprPO loss maximizes the separation between chosen and rejected trajectories along a preference direction $\mathbf{u}$. Crucially, this direction is defined in S-space, not the raw residual stream.

The preference direction $\mathbf{u}$ is computed as the normalized mean difference of projected activations:
$$\mathbf{u} = \text{normalize}\left( \mathbb{E}\left[ (h_{cho} - h_{rej}) V \right] \right)$$

Geometrically, $\mathbf{u}$ represents a weighting over the input channels ($V$ columns) that distinguishes honest from dishonest planning. The loss then maximizes the scalar projection of the policy's separation $\Delta h_\pi V$ onto this direction.

## 3. Justification for Projection Basis

A key design choice is projecting through $V$ rather than using the raw residual stream. The residual stream contains a superposition of features relevant to many different heads and layers. By projecting through $V$, we effectively "de-mix" this superposition, isolating only the subspace that the target layer ($W$) reads and amplifies.

While planning information is distributed across the residual stream, we target only the subset of information that flows through the specific layer being steered. This ensures that our intervention aligns with the layer's natural computational basis, modifying the signal in a way that is mechanistically consistent with the layer's function.
