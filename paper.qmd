...
- maybe we should call it the task direction not RHLF, RHLF seems broad when the effect may be also SFT, instruction tuning RLVR, RLAIF etc
- we should be honest about limitations, if we cherry pick, unrelaible etc
- we shouldn't assume our theories are true and pholosophise when we can report what we did and measured and observed


Consider add thing docs/img/innerpissa_architecture.svg for the classic lora arch img
And docs/img/loss_geometry_innerpissa.svg to give an intuition on the 3 part loss

Consider incorperating these minimal equations
...
However, for alignment debugging, the primary constraint is **robustness**, not FLOPs. As shown in @tbl-anti-rlhf, prompting fails catastrophically when steering against RLHF. InnerPiSSA trades a fixed parameter overhead for the capability to steer in this regime.
...
**Asymmetric RLHF resistance**: Steering against learned behaviors (coefficient = -1) degrades faster than steering with them (coefficient = +1), visible in coherence costs (TBD vs TBD NLL, @tbl-anti-rlhf). RLHF creates directional structure in representation space that resists reversal. Stronger anti-RLHF steering may require adversarial training during adapter optimization or alternate loss formulations that explicitly model resistance.
