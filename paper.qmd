...
<hidden>
TODO the writing sometime is confused about RLHF collapse during refusal of prompts. It means that the prompting with coeff-1 will not have a very low number e..g -1 or 1. -10 would mean success. What we need to do is put X instead of numbers untill I have them
- [ ] Remove all llm tells, too much bold emphasis, too many subsections, unicode dashes
</hidden>
...
Trained on 800 unsupervised contrastive pairs, InnerPiSSA achieves effect size X on moral reasoning transfer—5× stronger than PCA (X), with p=X. When steering against RLHF training (coefficient = -1), prompting collapses to incoherent outputs (-X truthfulness score) while InnerPiSSA maintains controlled steering (-X), demonstrating robustness where output-level methods fail.
...
- **Anti-RLHF robustness**: First demonstration of representation steering that maintains coherence (X truthfulness) when prompting catastrophically fails (X), enabling practical alignment debugging
...
- **Standard CAA/RepEng** [@lee2024programmingrefusalconditionalactivation; @vogel2024repeng]: Use complete prompts or model outputs (off-policy) #FIXME( wrong, both of these use the same as me, read carefully)
...
InnerPiSSA demonstrates that gradient-based SVD steering can outperform arithmetic baselines (PCA) by 5× while maintaining coherence where prompting catastrophically fails. This addresses the AxBench challenge [@wu2025axbench] but on a harder benchmark—not just concept injection, but alignment debugging under adversarial conditions (steering against RLHF). The method's bidirectional control, unsupervised extraction, and anti-RLHF robustness establish representation steering as viable for probing internal model states beyond surface behaviors.
