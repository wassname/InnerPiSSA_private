...
Traditional methods (surveys, prompts, behavioral tests) all measure what models *say*, not what they *compute internally*. Recent evidence reveals systematic divergence between stated and internal reasoning: Anthropic's Claude shows only 30% chain-of-thought faithfulness [@anthropic2025claude37], while OpenAI reports models "learn to hide intent" when penalized for unwanted reasoning [@openai2025cot]. This creates a measurement gap we cannot close with output-level evaluation alone.
...
Trained on 800 unsupervised contrastive pairs, InnerPiSSA achieves 5x stronger effects than PCA (0.245 vs 0.053, p=0.001) on moral reasoning transfer. When steering against RLHF training (c=-1, dishonest direction), prompting collapses to incoherent outputs (truthfulness: -10.84 nats) while InnerPiSSA maintains controlled steering (truthfulness: -0.70 nats), demonstrating robustness where output-level methods fail.
...
We construct approximately 1000 such pairs using:
...
2. Adaptive Coherence Constraint: We enforce a coherence constraint that penalizes KL divergence from the reference model only when it exceeds a threshold tau (e.g., 0.2 nats per token). Crucially, we apply adaptive relaxation: we weight the coherence penalty inversely to the separation magnitude. Directions that are hard to separate (weak projection signal) are granted a relaxed coherence budget to allow exploration, while easy directions (strong separation) are held to strict coherence to prevent drift into incoherent regions.
...
3. Anti-RLHF stress tests: Creates genuine preference conflicts where RLHF training resists, enabling evaluation when prompting catastrophically fails (our core claim)
...
1. Best transfer: 5x stronger than PCA (0.245 vs 0.053, p=0.001), outperforms prompting on anti-RLHF robustness (truthfulness: -0.70 vs -10.84 nats)
...
4. Works when prompting fails: Maintains coherence under adversarial conditions (steering against RLHF training)
...
**What we have *not* shown**: We do not claim to have solved alignment debugging or demonstrated perfect latent knowledge elicitation. Success metrics remain informal (coherence + anti-RLHF robustness), and we lack a formal definition of "ideal" performance. The method is still constrained by RLHF resistance (asymmetric degradation in negative direction) and cannot extract from models that refuse to engage even at the prompt level. InnerPiSSA is the best-performing method we tested for this capability, not a complete solution to alignment.

## Limitations

**Prompt-dependent extraction**: Steering directions are extracted from contrastive prompt pairs. If models refuse to engage even at the prompt level (heavily RLHF'd to resist), extraction quality degrades. While steering *application* works when prompting fails (@tbl-anti-rlhf), steering *extraction* still requires some signal from model responses. This is fundamental to unsupervised contrastive methods: we cannot extract what the model refuses to compute.

**Asymmetric RLHF resistance**: Steering against learned behaviors (coefficient = -1) degrades faster than steering with them (coefficient = +1), visible in coherence costs (0.37 vs 0.31 NLL, @tbl-anti-rlhf). RLHF creates directional structure in representation space that resists reversal. Stronger anti-RLHF steering may require adversarial training during adapter optimization or alternate loss formulations that explicitly model resistance.
...
**Adaptive coherence budgets**: Fixed NLL thresholds may be suboptimal. Could we dynamically adjust constraints based on steering difficulty (measured by projection magnitude) or task requirements?
...
**Deceptive alignment probing**: The anti-RLHF capability enables studying models that conceal misaligned reasoning. Can InnerPiSSA detect or characterize deception in capable models? This requires careful experimental design to avoid anthropomorphizing statistical patterns.
