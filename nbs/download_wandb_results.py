...
logger.info(
    f"Considering only runs after last major code change at {last_major_code_change}"
)
...
lastest_runs = list(
    api.runs(project, filters={"state": "finished", "created_at": {"$gt": last_run}})
)
...
for run in tqdm(lastest_runs):
    ...
    runs_data.append(run_data)
    logger.info(
        f"  {run.name} - metric={summary.get('eval/main_metric', np.nan):.3g}, created_at={run.created_at}"
    )
...
def fst_cnfg(df):
    ...
    if intro_dates:
        ...
        logger.warning(
            f"⚠️  Runs before {latest_intro} may be missing newer config options!"
        )
        return latest_intro
    return None
...
if "run_group" in df.columns and df["run_group"].notna().any():
    ...
    for group_name in df["run_group"].dropna().unique():
        ...
        logger.info(
            f"Group '{group_name}': {len(df_group)} runs, "
        ...
        group_summary.to_csv(
            group_summary_file, index=False, float_format="%.4g", na_rep="NA"
        )

    logger.info(
        f"Saved {len(df['run_group'].dropna().unique())} group summaries to {group_dir}"
    )
...
help_text = subprocess.run(
    ["uv", "run", "python", "nbs/train.py", ".", "-h"], capture_output=True, text=True
).stdout
