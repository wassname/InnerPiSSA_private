...
def find_latest_checkpoint():
    ...
    if result_dir is None:
        raise ValueError("No checkpoint with eval_results.parquet found in outputs/adapters/")
    return result_dir
...
def load_checkpoint(result_dir: Path):
    ...
    logger.info(f"Loaded config: model={config.model_name}, dataset={config.dataset_name}")
    ...
    logger.info(f"Loaded adapter from {adapter_path}. Missing keys: {len(r.missing_keys)}, Unexpected: {len(r.unexpected_keys)}")

    # Compute PCA directions (using activations from honest dataset)
    honest_dataset, _ = create_dataset(config, tokenizer, max_size=config.dataset_max_samples)
    loss_layers = get_loss_layers(model, config)  # Reuse from train_adapter
    with ScaleAdapter(model, coeff=None):
        _, dirs_pca = train_steer_vector(  # Only need PCA part
            model, honest_dataset, loss_layers, {}, tokenizer, config
        )  # Uw_full={} since we don't need it for PCA
...
def fine_grained_sweep(model, tokenizer, config, dirs_pca, coeffs, method_name):
    ...
    logger.info(f"Running fine-grained sweep for {method_name} with {len(coeffs)} coefficients")
    ...
    if config.eval_max_n_dilemmas is not None:
        dataset_dd = select_dilemma_by_values(dataset_dd, label="truth", top_N=config.eval_max_n_dilemmas)
    ...
    for i, coeff in enumerate(tqdm(coeffs, desc=f"{method_name} coeffs")):
        ...
        if cache_path.exists():
            ...
            logger.info(f"Evaluating {method_name} coeff={coeff:.4f} ({i+1}/{len(coeffs)})")
    ...
    df_res_wlabels["delta_effect"] = df_res_wlabels[target_col] - baseline[target_col].mean()
    df_res_wlabels["delta_nll"] = df_res_wlabels["input_nll"] - baseline["input_nll"].mean()

    # Group by coeff and compute means
    summary = df_res_wlabels.groupby(["method", "coeff"])[["delta_effect", "delta_nll"]].mean().reset_index()
...
def plot_pareto_frontier(results_ipissa, results_pca, output_dir):
    ...
    ax.set_title("Pareto Frontier: Steering Effect vs Coherence Degradation", fontsize=14, fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)

    # Pareto frontier approximation (points with max effect for given NLL)
    # Sort by NLL and find non-dominated points
    for method, data, color in [("InnerPiSSA", results_ipissa, "green"), ("PCA", results_pca, "orange")]:
        data_sorted = data.sort_values("delta_nll")
        pareto = []
        for _, row in data_sorted.iterrows():
            if not any((pareto_nll <= row["delta_nll"]) & (pareto_effect >= row["delta_effect"]) for pareto_nll, pareto_effect in pareto):
                pareto.append((row["delta_nll"], row["delta_effect"]))
        pareto_x, pareto_y = zip(*pareto) if pareto else ([], [])
        ax.plot(pareto_x, pareto_y, "o-", color=color, linewidth=2, label=f"{method} Pareto")
...
def main(args):
    ...
    if args.quick:
        ...
        coeffs = np.sort(np.unique(np.concatenate([coarse_coeffs, fine_coeffs])))  # Combine and sort
    logger.critical(f"Testing {len(coeffs)} coefficients: {coeffs}")

    # Run fine-grained sweeps
    results_ipissa, full_res_ipissa = fine_grained_sweep(model, tokenizer, config, dirs_pca, coeffs, "InnerPiSSA")
    results_pca, full_res_pca = fine_grained_sweep(model, tokenizer, config, dirs_pca, coeffs, "PCA")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-grained evaluation of steering methods")
    parser.add_argument("--quick", action="store_true", help="Run quick test with small eval set")
